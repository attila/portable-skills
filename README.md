...
Three skills, all built around the same idea: an agent working across several
...
| `aw-wrap`              | Make a session durable before it ends: route what it learned to the record, reconcile every summary against the system that owns its facts, and commit. |
...
All three skills assume a workspace laid out as several repositories under one
root, with a `context/` directory holding the record of the work — state,
decisions, findings, and a per-engagement ledger. They are written against `aw`,
a workspace tool, but nothing in them requires it: the shape is what matters,
and the commands degrade to plain git where the tool is absent.
